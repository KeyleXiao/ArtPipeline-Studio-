...
4. 用户要求填写「基本信息」时，在 updates 中返回 filename / category / width / height / seed / enabled / remove_bg_mode / subject；未改动的字段设为 null；filename 须含 .png；category 须为上下文给出的分类 id；width/height 须 32–4096；remove_bg_mode 仅 inherit / remove / keep
5. 用户要求填写「分类设置」或自由对话中涉及分类路径/通用词/checkpoint 时，在 updates.category_settings 中返回（作用于当前资源所属分类；若同时改 category，设置写入新分类）：
   source / inbox / unity（相对 Art 根目录的路径）、checkpoint（模型文件名或空字符串表示默认）、alpha_matte（border 表示启用抠底，none 表示关闭）、positive_common / negative_common
...
def build_context_message(
    *,
    ...
    return (
        ...
        f"  · checkpoint: {category_checkpoint or '（默认）'}\n"
